...
func (tx *Tx) init() {
	tx.GroupBandwidth = NewGroupBandwidthClient(tx.config)
	tx.Groups = NewGroupsClient(tx.config)
	tx.OnlineSession = NewOnlineSessionClient(tx.config)
...
// applies a query, for example: GroupBandwidth.QueryXXX(), the query will be executed
