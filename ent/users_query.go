...
func (uq *UsersQuery) Clone() *UsersQuery {
	...
	return &UsersQuery{
		...
		withInGroup: uq.withInGroup.Clone(),
...
// WithInGroup tells the query-builder to eager-load the nodes that are connected to
// the "in_group" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UsersQuery) WithInGroup(opts ...func(*GroupsQuery)) *UsersQuery {
	query := &GroupsQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withInGroup = query
	return uq
}
...
func (uq *UsersQuery) sqlAll(ctx context.Context) ([]*Users, error) {
	...
	if query := uq.withInGroup; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Users)
		for i := range nodes {
			if nodes[i].groups_users == nil {
				continue
			}
			fk := *nodes[i].groups_users
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "groups_users" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.InGroup = n
			}
		}
	}
