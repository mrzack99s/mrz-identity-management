// Code generated by entc, DO NOT EDIT.

package ent

import (
	"context"
	"fmt"

	"entgo.io/ent/dialect/sql"
	"entgo.io/ent/dialect/sql/sqlgraph"
	"entgo.io/ent/schema/field"
	"github.com/mrzack99s/mrz-identity-management/ent/groupbandwidth"
	"github.com/mrzack99s/mrz-identity-management/ent/predicate"
)

// GroupBandwidthDelete is the builder for deleting a GroupBandwidth entity.
type GroupBandwidthDelete struct {
	config
	hooks    []Hook
	mutation *GroupBandwidthMutation
}

// Where appends a list predicates to the GroupBandwidthDelete builder.
func (gbd *GroupBandwidthDelete) Where(ps ...predicate.GroupBandwidth) *GroupBandwidthDelete {
	gbd.mutation.Where(ps...)
	return gbd
}

// Exec executes the deletion query and returns how many vertices were deleted.
func (gbd *GroupBandwidthDelete) Exec(ctx context.Context) (int, error) {
	var (
		err      error
		affected int
	)
	if len(gbd.hooks) == 0 {
		affected, err = gbd.sqlExec(ctx)
	} else {
		var mut Mutator = MutateFunc(func(ctx context.Context, m Mutation) (Value, error) {
			mutation, ok := m.(*GroupBandwidthMutation)
			if !ok {
				return nil, fmt.Errorf("unexpected mutation type %T", m)
			}
			gbd.mutation = mutation
			affected, err = gbd.sqlExec(ctx)
			mutation.done = true
			return affected, err
		})
		for i := len(gbd.hooks) - 1; i >= 0; i-- {
			if gbd.hooks[i] == nil {
				return 0, fmt.Errorf("ent: uninitialized hook (forgotten import ent/runtime?)")
			}
			mut = gbd.hooks[i](mut)
		}
		if _, err := mut.Mutate(ctx, gbd.mutation); err != nil {
			return 0, err
		}
	}
	return affected, err
}

// ExecX is like Exec, but panics if an error occurs.
func (gbd *GroupBandwidthDelete) ExecX(ctx context.Context) int {
	n, err := gbd.Exec(ctx)
	if err != nil {
		panic(err)
	}
	return n
}

func (gbd *GroupBandwidthDelete) sqlExec(ctx context.Context) (int, error) {
	_spec := &sqlgraph.DeleteSpec{
		Node: &sqlgraph.NodeSpec{
			Table: groupbandwidth.Table,
			ID: &sqlgraph.FieldSpec{
				Type:   field.TypeInt,
				Column: groupbandwidth.FieldID,
			},
		},
	}
	if ps := gbd.mutation.predicates; len(ps) > 0 {
		_spec.Predicate = func(selector *sql.Selector) {
			for i := range ps {
				ps[i](selector)
			}
		}
	}
	return sqlgraph.DeleteNodes(ctx, gbd.driver, _spec)
}

// GroupBandwidthDeleteOne is the builder for deleting a single GroupBandwidth entity.
type GroupBandwidthDeleteOne struct {
	gbd *GroupBandwidthDelete
}

// Exec executes the deletion query.
func (gbdo *GroupBandwidthDeleteOne) Exec(ctx context.Context) error {
	n, err := gbdo.gbd.Exec(ctx)
	switch {
	case err != nil:
		return err
	case n == 0:
		return &NotFoundError{groupbandwidth.Label}
	default:
		return nil
	}
}

// ExecX is like Exec, but panics if an error occurs.
func (gbdo *GroupBandwidthDeleteOne) ExecX(ctx context.Context) {
	gbdo.gbd.ExecX(ctx)
}
